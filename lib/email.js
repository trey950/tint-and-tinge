...
export async function sendAnalysisEmail({ to, name, result, orderRef }) {
  ...
  if (r?.error) {
    ...
    console.error("[email] Resend rejected send to", to, ":", msg);
    throw new Error("Resend: " + msg);
  }
  return r;
}

export async function sendAbandonedCheckoutEmail({ to, name }) {
  if (!resend) {
    console.warn("[email] RESEND_API_KEY not set — skipping abandon reminder");
    return { id: "dry-run-no-key" };
  }
  const first = (name || "").trim().split(/\s+/)[0] || "there";
  const subject = `${first}, your color palette is one step away`;
  const html = renderAbandonHtml({ name });
  const r = await resend.emails.send({
    from: `Tint & Tinge <${FROM}>`,
    to,
    replyTo: REPLYTO,
  ...
  if (r?.error) {
    const msg = r.error.message || JSON.stringify(r.error);
    console.error("[email] Resend rejected abandon reminder to", to, ":", msg);
    throw new Error("Resend: " + msg);
  }
  return r;
}

function renderAbandonHtml({ name }) {
  const first = (name || "").trim().split(/\s+/)[0];
  const hi = first ? `${esc(first)},` : "Hello,";
  ...
  <tr><td align="center" style="padding:40px 16px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="560" style="max-width:560px;background:#ffffff;border:1px solid ${LINE};border-radius:16px;overflow:hidden;">
      <tr><td style="padding:24px 32px;border-bottom:1px solid ${LINE};font-family:Georgia,serif;font-size:18px;color:${INK};">Tint &amp; Tinge</td></tr>
      <tr><td style="padding:36px 32px 8px;">
        <div style="font-family:Georgia,serif;font-size:30px;line-height:1.2;color:${INK};">${hi} your palette is waiting.</div>
      </td></tr>
      <tr><td style="padding:8px 32px 0;font-family:Arial,sans-serif;font-size:15px;line-height:1.6;color:${ACCENT};">
        You started your personal color analysis but didn't quite finish. Your answers are saved — pick up right where you left off and we'll email your full 12-season report within minutes of checkout.
      </td></tr>
      <tr><td style="padding:20px 32px 4px;font-family:Arial,sans-serif;font-size:14px;line-height:1.7;color:${ACCENT};">
        Your report includes your exact season, a 15–20 color palette with hex codes, colors to avoid, your foundation neutrals, and a tailored edit for metals, prints, makeup, <strong>clothing and jewelry</strong>.
      </td></tr>
      <tr><td align="center" style="padding:24px 32px 8px;">
        <a href="${APP_URL}" style="display:inline-block;background:#2a2823;color:${CREAM};text-decoration:none;font-family:Arial,sans-serif;font-size:15px;font-weight:600;padding:14px 28px;border-radius:999px;">Finish my analysis →</a>
      </td></tr>
      <tr><td align="center" style="padding:0 32px 28px;font-family:Arial,sans-serif;font-size:12px;color:${MUTED};">
        $49, one-time · In-person analysis costs $150–300 · Love it or your money back, 7 days
      </td></tr>
      <tr><td style="background:${CREAM};padding:20px 32px;border-top:1px solid ${LINE};text-align:center;font-family:Arial,sans-serif;font-size:11px;color:${MUTED};line-height:1.6;">
        You received this because you started a color analysis at ${esc(APP_URL)}.<br/>
        ${esc(BUSINESS_NAME)} · ${esc(BUSINESS_ADDRESS)}<br/>
        Questions? <a href="mailto:${esc(SUPPORT_EMAIL)}" style="color:${ROSE_DEEP};">${esc(SUPPORT_EMAIL)}</a>
      </td></tr>
    </table>
  ...
  <tr><td align="center" style="padding:40px 16px;">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600" style="width:600px;max-width:600px;background:#ffffff;border:1px solid ${LINE};border-radius:18px;overflow:hidden;">
      ...
      <tr><td style="padding:0 32px;">
        ...
        <div style="background:${CREAM};border:1px solid ${LINE};border-radius:12px;padding:20px;margin-bottom:12px;">
          <div style="font-family:Arial,sans-serif;font-size:9px;color:${ROSE_DEEP};font-weight:700;letter-spacing:2px;text-transform:uppercase;">Clothing &amp; wardrobe</div>
          <div style="font-family:Georgia,serif;font-size:17px;color:${INK};font-weight:600;margin:4px 0 8px;">${esc(meta.clothing.title)}</div>
          <div style="font-family:Arial,sans-serif;font-size:13px;color:${ACCENT};line-height:1.55;">${esc(meta.clothing.text)}</div>
        </div>

        <div style="background:${CREAM};border:1px solid ${LINE};border-radius:12px;padding:20px;">
          <div style="font-family:Arial,sans-serif;font-size:9px;color:${ROSE_DEEP};font-weight:700;letter-spacing:2px;text-transform:uppercase;">Jewelry</div>
          <div style="font-family:Georgia,serif;font-size:17px;color:${INK};font-weight:600;margin:4px 0 8px;">${esc(meta.jewelry.title)}</div>
          <div style="font-family:Arial,sans-serif;font-size:13px;color:${ACCENT};line-height:1.55;">${esc(meta.jewelry.text)}</div>
        </div>
      </td></tr>
